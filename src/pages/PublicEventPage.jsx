...
const PublicEventPage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Autorização enviada com sucesso! Você receberá um e-mail de confirmação.');
    },
    onError: (error) => {
      toast.error(error.response?.data?.detail || 'Falha ao enviar autorização.');
    },
  });
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      autorizacao_id: Yup.string().required('Por favor, selecione o nome do aluno na lista.'),
          ...
          <div>
            ...
            >
              ...
              {students?.map(student => (
                <option key={student.id} value={student.id}>{student.nome_aluno}</option>
              ))}
            </select>
