  ...
  useEffect(() => {
    if (event) {
      formik.setValues({
        ...
        campus_id: event.campus_id || '', // Adicionado
      });
    }
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Editar Evento">
      {isLoadingCampuses ? <LoadingSpinner /> : (
        <form onSubmit={formik.handleSubmit} className="space-y-4">
          {/* 4. Adicionar campo de seleção */}
          <div>
            <label htmlFor="edit_campus_id" className="block text-sm font-medium text-gray-700">Campus</label>
            <select
              id="edit_campus_id"
